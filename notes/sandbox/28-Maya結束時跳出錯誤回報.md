...
|---|---|---|
| 每格時間 | 約 22 秒 | 14～15 秒 |
| 授權模組當機（`cer.log` 的 `launching ... senddmp.exe`） | 每格一次 | **0 次** |
| 錯誤回報視窗 | 每格一個 | 沒有 |

判斷有沒有當機要看 `senddmp`，不能只看 `cer started`：授權模組每次載入都會初始化 CER、寫下 `cer started`，
之後的 Arnold 算圖（[`30`](30-Arnold授權與假成功.md)）每格都有這一行，但沒有例外碼、沒有 dump、也沒有視窗。
