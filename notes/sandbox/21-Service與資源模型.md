...
1. **RQD 會拿它建立同名使用者**來執行 frame（見 [`04` 坑 #4](04-部署過程.md#坑-4rqd-建立使用者失敗uid-衝突) 的 uid 衝突）
...
**它會硬性限制派工**，而且失敗時完全沒有錯誤訊息 —— 見 [`12` 的坑 #17](12-真實DCC算圖.md#坑-17facility-不匹配會靜默地不派工)
與 [`17` 的場景 20](17-故障排查與常見疏失速查.md#場景-20新節點加入後永遠收不到工作)。
...
而 [`17` 的場景 1](17-故障排查與常見疏失速查.md#場景-1job-卡在-pendingframe-全卡-waitinghost-顯示-up--open)、[19](17-故障排查與常見疏失速查.md#場景-19job-被派到沒有該-dcc-版本的節點)、[20](17-故障排查與常見疏失速查.md#場景-20新節點加入後永遠收不到工作) 分別對應其中三種原因。
